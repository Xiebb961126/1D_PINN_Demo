...
class OneDPINN_ad(nn.Module):
    ...
    def forward(self, x: torch.FloatTensor, area: torch.FloatTensor, measure_v: list,
        ...
        detaP = torch.autograd.grad(outputs=output_tensor[:, :, 1].sum(), inputs=x, retain_graph=True)[0]
        detaQ = torch.autograd.grad(outputs=output_tensor[:, :, 1].sum(), inputs=x, retain_graph=True)[0]

        loss_meas = loss_measure(output_tensor, input_tensor, measure_v, measure_p, p_norm, Q_norm)
        loss_pde_mass, loss_pde_mom = loss_pde(output_tensor, input_tensor, detaP, detaQ, p_norm, Q_norm,
...
def loss_pde(output_tensor, input_tensor, detaP, detaQ, p_norm, Q_norm, rho=1060, mu=0.004, finite_num=20):
    """
        :param output_tensor: (vessel_num, point_num, [u, p])
        :param input_tensor: (vessel_num, point_num, [x, are])
        :param density: 密度
    ...
    deta_pressure_in = detaP[:, range_ids]
    term1 = deta_pressure_in * area_in
    term1 = term1

    deta_Q_in = detaQ[:, range_ids]
    deta_area_in = (area_out - area_in) / (x_out - x_in)  # 前向差分
    term2 = 3 / 4 * (2 * output_tensor[:, range_ids, 0] * deta_Q_in -
                     output_tensor[:, range_ids, 0] ** 2 * deta_area_in)
    term2 = term2

    term_3 = func_N.unsqueeze(-1) * Q_in / area_in

    loss_func = nn.MSELoss(reduction="mean")
    loss_residual_momentum = (term1 + term2 + term_3) / p_norm
...
if __name__ == "__main__":
    ...
    onepinn = OneDPINN_ad(vessel_number=vessel_number, finite_number=finite_number, setting_list=layers, act_func="relu")
