...
class OneDPINN(nn.Module):
    ...
    def init_network_layer(self):
        """
            为每个血管段创建一个 FULL CONNECTION。生成整个冠脉树网络
        :return:
        """
        for vessel_number in range(self.vessel_number):
            vessel_list = nn.Sequential()
            num_setting = len(self.layer_setting)
            for i, (in_dim, out_dim) in enumerate(self.layer_setting):
                vessel_list.add_module('conv_v%d_l%d' % (vessel_number, i), nn.Linear(in_dim, out_dim))
                if i < num_setting - 1:
                    vessel_list.add_module('act_v%d_l%d' % (vessel_number, i), self.select_activate_function())
            self.model_list.append(vessel_list)
    ...
    def forward(self, x: torch.FloatTensor, area: torch.FloatTensor, measure_v: list,
        ...
        output_tensor = []
        for vessel_id, model in enumerate(self.model_list):
            output_tensor.append(model(nor_input_tensor[vessel_id]))
        output_tensor = torch.stack(output_tensor, dim=0)  # vessel_num, point_num, [u, p]
...
if __name__ == "__main__":
    ...
    onepinn = OneDPINN(vessel_number=vessel_number, finite_number=finite_number, setting_list=layers, act_func="relu")
